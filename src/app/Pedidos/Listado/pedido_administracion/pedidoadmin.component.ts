...
export class PedidoAdminComponent implements OnInit {
  ...
  @ViewChild('mConfirmarPedidoNoEntregado') mConfirmarPedidoNoEntregado: TemplateRef<any>;
  ...
  generateMapPoints() {
    ...
    if( this.mainObj$.tipo_servicio === 1 ){
      this.mapPoints[1].icon = {
        "url" : "https://www.codigeek.com/projects/robertodelivery/car.png",
        scaledSize: {
          width: 40,
          height: 32
        }
      ...
      if( this.mainObj$.repartidor_latitude ){
        this.mapPoints.push({
          ...
          "icon" : {
            "url" : "https://www.codigeek.com/projects/robertodelivery/delivery.png",
            scaledSize: {
              width: 35,
              height: 40
            }
  ...
  abrirModalConfirmacionNoEntregado(){
    this.modalReference = this.modalService.open(this.mConfirmarPedidoNoEntregado);
  }

  actualizarPedidoNoEntregado(){
    if( !this.mainObj$.tipo_cancelacion ){
      this.dataService.generalAlert({ "status" : "info" , "message" : "Completa el tipo de cancelación" });
      return;
    }
    this.mainObj$.fecha_cancelacion = new Date();
    this.dataService.useService( "actualizar_pedido_cancelar" , { data : this.mainObj$ } )
    .subscribe
      (
        (data : any) =>   {
          this.dataService.generalAlert(data);
