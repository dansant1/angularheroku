...
export class PerfilRestauranteComponent implements OnInit {
  ...
  regresar(){
    this.router.navigateByUrl('/cliente/seleccion-restaurante', {  });
  }
  ...
  opcionesPlatillo( platillo_t ){
    ...
    this.platilloSeleccionadoVista$ = JSON.parse(JSON.stringify(platillo_t));
  ...
  agregarPlatillo( platillo_t ){
    ...
    localStorage.setItem('pedido_robertodelivery_App', JSON.stringify(this.mainObject$));
  ...
  ngOnInit() {
    ...
    if( history.state.negocio ){
      if( history.state.negocio._id ){
        ...
        if( !this.negocioObj$.banner_image ){
          this.negocioObj$.banner_image = "https://www.codigeek.com/projects/robertodelivery/background_negocio.png";
        }
      }else{
    ...
    if( history.state.descuento_puntos ){
      this.mainObject$.descuento = history.state.puntos.puntos;
    }
